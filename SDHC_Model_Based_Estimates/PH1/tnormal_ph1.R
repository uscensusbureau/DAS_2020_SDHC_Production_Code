##############################################################################
# Table PH1 (formerly P17): Average Household Size by Age
#
  ...
  for (idx_geo in geoids) {
    for (idx_it in iteration_codes) {
      ...
      u_bd = c(NUM2 + 6*sqrt(NUM2_VAR),
               ...
               DEN + 6*sqrt(DEN_VAR),
               NUM2 + NUM3 - DEN + 6 * sqrt(NUM2_VAR + NUM3_VAR + DEN_VAR),
      ...
      init_val = quadprog::solve.QP(Dmat=diag(3),
                                    dvec=c(NUM2, NUM3, DEN),
                                    t(Dstar), bdstar)$solution
      tmp = try(rtmvn(10000, Mean=c(NUM2, NUM3, DEN), Sigma=SIG, D=D,
      ...
      data$PRED_NUM[w1] = mean(tmp[, 1] + tmp[, 2])
      data$PRED_NUM[w2] = mean(tmp[, 1])
      data$PRED_NUM[w3] = mean(tmp[, 2])
